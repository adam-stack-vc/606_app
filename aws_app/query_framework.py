...
# -----------------------------
# Venue Alias Utilities
# -----------------------------
def load_venue_aliases(filename: str = "venue_aliases.json") -> dict:
    path = os.path.join(os.path.dirname(__file__), filename)
    with open(path, "r") as f:
        return json.load(f)

def resolve_venue(user_input: str, alias_map: dict) -> str:
    lowered = user_input.lower()
    # Handle nested structure where aliases are under "venue_map" key
    venue_map = alias_map.get("venue_map", alias_map)
    for canonical, aliases in venue_map.items():
        if any(alias.lower() in lowered for alias in aliases):
            return canonical
    return ""
...
def classify_query(user_input: str) -> dict:
    ...
    venue_alias_map = load_venue_aliases()
    resolved_venue = resolve_venue(user_input, venue_alias_map)
    if resolved_venue:
