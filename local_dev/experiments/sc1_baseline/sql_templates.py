...
def _metric_select(table: str, intent: QueryIntent) -> str:
    ...
    # P1 FIX: Handle COUNT(DISTINCT column) for aggregate_with_count operations
    if agg == "COUNT" and metric:
        # Normalize column names
        column = metric
        if column == "venue":
            column = "venues"  # Normalize to actual column name
        elif column == "broker":
            column = "executing_bd"

        # Generate COUNT(DISTINCT ...) with appropriate alias
        count_alias = f"{column}_count"
        return f"COUNT(DISTINCT {_safe_ident(column)}) AS {count_alias}"
...
def build_aggregate(intent: QueryIntent, table: str) -> str:
    ...
    # Handle dimensions using capability-driven approach
    dims = []
    temporal_dims = []
    explicit_temporal_cols = []  # Track explicit temporal columns
    ...
    # P1 FIX: For COUNT operations, filter out NULL and empty values
    if intent.aggregation and intent.aggregation.upper() == "COUNT" and intent.metric:
        count_column = intent.metric
        if count_column == "venue":
            count_column = "venues"
        elif count_column == "broker":
            count_column = "executing_bd"
        where.append(f"{count_column} IS NOT NULL")
        where.append(f"{count_column} != ''")
        # For executing_bd_606, also filter by data_type='venue' when counting venues
        if table == "executing_bd_606" and count_column == "venues":
            where.append("data_type = 'venue'")
