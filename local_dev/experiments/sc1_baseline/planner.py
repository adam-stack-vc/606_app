...
def _detect_operation_and_dims(tags: Dict) -> Tuple[str, list]:
    ...
    # Must be specific: "across the system", "across all tables", not just "across"
    if re.search(r'\b(entities|names|participants)\b.*\b(across.*?(system|tables)|system|all tables)\b', text) or \
       re.search(r'\b(across.*?(system|tables)|system)\b.*\b(entities|names|participants)\b', text):
    ...
    # P0 FIX: "for each X, count/how many Y" pattern - MUST BE BEFORE "list" pattern
    # Examples: "for each broker, how many venues", "each broker received PFOF from how many venues"
    # This needs aggregate with GROUP BY, not standalone count
    ...
    # Also detect implicit grouping: "broker...how many venues" or "venue...how many brokers"
    ...
    if has_count_request and (has_grouping or has_broker_and_venue):
        # Extract the grouping entity (what to GROUP BY)
        # Priority: explicit "for each X" patterns
        if re.search(r'\b(for each|each)\s+(broker|brokers?|executing.?bd)\b', text):
            dims = ["executing_bd"]
            return "aggregate_with_count", dims
        elif re.search(r'\bper\s+(broker|brokers?|executing.?bd)\b', text):
            dims = ["executing_bd"]
            return "aggregate_with_count", dims
        elif re.search(r'\b(for each|each|per)\s+(venue|venues?)\b', text):
            dims = ["venues"]
            return "aggregate_with_count", dims
        elif re.search(r'\b(for each|each|per)\s+(market participant|participant)\b', text):
            dims = ["market_participant"]
            return "aggregate_with_count", dims
        # NEW: Temporal grouping patterns - "in each month", "for each quarter", etc.
        elif re.search(r'\b(in|for|per)\s+each\s+(month|quarter|year)\b', text):
            match = re.search(r'\b(in|for|per)\s+each\s+(month|quarter|year)\b', text)
            temporal_dim = match.group(2) if match else None
            if temporal_dim:
                dims = [temporal_dim]
                return "aggregate_with_count", dims
        # Implicit: if asking about brokers AND venues with count, group by broker
        elif has_broker_and_venue:
            # Default: if counting venues, group by broker; if counting brokers, group by venue
            if "how many venue" in text or "number of venue" in text or "count.*venue" in text:
                dims = ["executing_bd"]
                return "aggregate_with_count", dims
            elif "how many broker" in text or "number of broker" in text:
                dims = ["venues"]
                return "aggregate_with_count", dims
    ...
    # P1 FIX: "list...and count/number of" should use aggregate, not standalone list
    # Example: "list every broker and the number of venues"
    if any(w in text for w in ["list", "show", "display"]):
        if has_count_request:
            # This is aggregate with both entity list and count
            return "aggregate_with_count", dims
        # Pure list/distinct without counting
        if any(w in text for w in ["unique", "distinct", "all"]):
            return "list", dims
...
def _build_sql_from_intent(intent: QueryIntent, table: str, user_input: str) -> Optional[str]:
    ...
    # P0/P1 FIX: Aggregate with COUNT - "for each X, count Y"
    if op == "aggregate_with_count":
        # Extract what to count from the query
        text_lower = user_input.lower()
        count_target = None

        # Determine what we're counting
        if "venue" in text_lower and "executing_bd" in (intent.dimensions or []):
            count_target = "venues"
        elif "broker" in text_lower and "venues" in (intent.dimensions or []):
            count_target = "executing_bd"
        elif "venue" in text_lower:
            count_target = "venues"
        elif "broker" in text_lower or "executing" in text_lower:
            count_target = "executing_bd"

        if count_target and intent.dimensions:
            # Build aggregate query with COUNT(DISTINCT) as the metric
            # Override the intent to use COUNT aggregation
            intent.metric = count_target
...
def plan_single_sql(user_input: str, tags: Dict) -> Optional[str]:
    ...
    if op == "aggregate_with_count":
        ...
        # Determine what we're counting based on what's mentioned
        if "how many venue" in text_lower or "number of venue" in text_lower:
            count_target = "venues"
        elif "how many broker" in text_lower or "number of broker" in text_lower:
            count_target = "executing_bd"
        elif "venue" in text_lower and "executing_bd" in (intent.dimensions or []):
            count_target = "venues"
        elif "broker" in text_lower and "venues" in (intent.dimensions or []):
            count_target = "executing_bd"
        ...
        if count_target and intent.dimensions:
            ...
            # E.g., if counting venues, remove intent.entities["venue"]
